        ...
        <ul className='companies'>
          {
            companies.map(comp => {
              return(
                  <li key={comp.id} className={comp.id === location.hash.slice(1) ? 'selected': null}>
        ...
        <ul className='compInfo'>
        {
          profits.length > 0 ?
              profits.map(prof => {
                return ( 
                <li key={ prof.id }>
                  <span>{ prof.id } Was founded on { prof.createdAt }</span>
                  <br />
                  <span>Earnings for the year of { prof.fiscalYear.slice(0, 4) } was estimated at: 
                  <br /> 
                  { prof.amount }</span>
                </li> 
                )
